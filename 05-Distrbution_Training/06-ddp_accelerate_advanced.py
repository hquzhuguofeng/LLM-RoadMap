...
def train(model, optimizer, trainloader, validloader, accelerator: Accelerator, resume, epoch=3, log_step=10):
    ...
    if resume is not None:
        ...
        steps_per_epoch = math.ceil(len(trainloader) / accelerator.gradient_accumulation_steps) # 计算一个epoch训多少个样本
        resume_step = global_step = int(resume.split("step_")[-1]) # 获取当前模型的global_step
        resume_epoch = resume_step // steps_per_epoch # 计算当前保存模型进行的epoch轮数
        resume_step -= resume_epoch * steps_per_epoch # 计算当前轮数下的step数值
        accelerator.print(f"resume from checkpoint -> {resume}")

    # 注意这个range的起始是从resume_epoch开始的
...
def main():
    ...
    train(model, optimizer, trainloader, validloader, accelerator, resume="ckpts/step_150")
