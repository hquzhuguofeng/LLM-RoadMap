...
class Server:
    ...
    def __init__(self, name: str, config: Dict[str, Any]) -> None:
        ...
        self.sse_context: Optional[Any] = None
        # 服务器的会话，用于发送请求和接收响应
    ...
    async def initialize(self) -> None:
        ...
        if not url:
            raise ValueError(f"服务器 {self.name} 的配置中未提供 SSE URL")
        try:
            # 建立 SSE 连接并保持流
            self.sse_context = streamablehttp_client(url=url)
            # 保存上下文以便后续清理
            read_stream, write_stream, get_session_id_callback = await self.sse_context.__aenter__()
